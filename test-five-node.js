...
function readFirstFiveRecords() {
  ...
  console.log("Using ONLY the first 5 records.");
...
function buildDocument(row, index) {
  ...
  const docKey =
    parts.length > 0
      ? parts.join("_")
      : `SF-CUSTOMER-COMPLAINT-TEST-${index + 1}`;
...
async function sendFiveRecords(token, documents) {
  const payload = {
    documents: documents,
    sync_source: "Salesforce Customer Feedback - Test Five"
  };

  console.log("\n========================================");
  console.log("SENDING FIRST 5 RECORDS");
  console.log("========================================");

  console.log(`Total records being sent: ${documents.length}`);
  console.log(`Endpoint: ${BATCH_URL}`);
  ...
  if (response.ok) {
    console.log("\nSUCCESS: First 5 records were accepted.");
  } else {
    console.log("\nFAILED: DocuFlow rejected the request.");
  }
}
...
async function main() {
  try {
    console.log("========================================");
    console.log("DOCUFLOW TEST - FIRST 5 RECORDS");
    console.log("SALESFORCE CUSTOMER COMPLAINT");
    console.log("========================================");

    const token = await login();

    const rows = readFirstFiveRecords();

    const documents = rows.map((row, index) => {
      console.log(
        `Preparing record ${index + 1}/5`
      );
    ...
    documents.forEach((document, index) => {
      console.log(
        `${index + 1}. ${document.DocKey}`
      );
    });

    await sendFiveRecords(token, documents);
